      ...
      for (i = 0; i < core_symb_vect->reduces.len; i++)
	{
	  ...
	  if (parent_anode != NULL && disp >= 0)
	      ...
	      if (n_candidates != 0)
		{
	      ...
	      int nterm_was = 0;
	      int pt_nterm_pos;
	      //printf("%s:", sit_rule->lhs->repr);
	      if(!pt_root_node->repr)
		pt_node->repr = sit_rule->lhs->repr;
	      pt_node->childs_num = sit_rule->rhs_len;
	      pt_node->childs = malloc( sizeof(struct pt_node*) * pt_node->childs_num);
		...
		if(sit_rule->rhs[bb]->term_p) {
		  ...
		  switch(sit_rule->rhs[bb]->u.term.code) 
		    {
		      case '/':
			asprintf(&(pt_node->childs[bb]->repr), "SLASH");
			break;
		      case ' ':
			asprintf(&(pt_node->childs[bb]->repr), "SPACE");
			break;
		      case '\n':
			asprintf(&(pt_node->childs[bb]->repr), "NEWLINE");
			break;
		      case '\t':
			asprintf(&(pt_node->childs[bb]->repr), "TAB");
			break;
		      default:
		  ...
		  nterm_was = 1;
		  bb_nterm_pos = bb;
		  pt_child_node = pt_node->childs[bb];
		  //printf(" %s", sit_rule->rhs[bb]->repr);
		  pt_node->childs[bb]->type = 0;
		  pt_node->childs[bb]->repr = sit_rule->rhs[bb]->repr;
		}
	      }
	      //printf ("\n");
	      if(nterm_was) 
	        {
		  pt_node = pt_child_node;
		  pt_nterm_pos = bb_nterm_pos;
		  pt_depth++;
		  OS_TOP_ADD_MEMORY (pt_pos, &pt_nterm_pos, sizeof (int));
	        }
	      else 
	        {
		  int* a;
		  while(pt_node->parent != NULL)
		    {
		      OS_TOP_SHORTEN(pt_pos, sizeof(int));
		      a = OS_TOP_BOUND(pt_pos);
		      //printf("num is: %d\n", *a);
		      if(*a)
			{
			  for(bb = *a-1; bb >= 0; bb--) 
			    {
			      //go to previous nterm
			      //printf("pt_node_repr: %s\n", (pt_node->parent->childs[bb])->repr);
			      if(!pt_node->parent->childs[bb]->type)
				{
				  pt_node = pt_node->parent->childs[bb];
		  		  OS_TOP_ADD_MEMORY (pt_pos, &bb, sizeof (int));
				  //printf("return to %s, parent is %s \n", pt_node->repr, pt_node->parent->repr);
				  break;
				}
			      if(bb == 0) 
				{
				  pt_node = pt_node->parent;
				  //printf("return2 to parent %s\n", pt_node->repr);
				}
			    }
			  if( bb != -1)
			    break;
		        ...
		        {
			  pt_node = pt_node->parent;
			  //printf("return to parent %s\n", pt_node->repr);
			  //printf("pt_node_parent: %s\n", pt_node->repr);
			}
		    }
